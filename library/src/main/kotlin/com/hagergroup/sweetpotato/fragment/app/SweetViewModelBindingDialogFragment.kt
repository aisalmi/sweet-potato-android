 ...
 * @since 2018.11.07
 */
abstract class SweetViewModelBindingDialogFragment<AggregateClass : SweetFragmentAggregate, BindingClass : ViewDataBinding>
  : SweetDialogFragment<AggregateClass>()
{
  ...
  override suspend fun onRetrieveModel()
  {
    ...
    viewModel = if (getAggregate()?.getViewModelContextFromAnnotation() == SweetFragmentAnnotation.ViewModelContext.Fragment)
    {
      ...
      {
        ViewModelProvider(this@SweetViewModelBindingDialogFragment, viewModelFactory).get(viewModelClass)
      }
      else
      {
        ViewModelProvider(this@SweetViewModelBindingDialogFragment).get(viewModelClass)
      }
    }
    else
    {
